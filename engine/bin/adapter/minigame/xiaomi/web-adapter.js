...
function (module, exports, __webpack_require__) {
  ...
  function inject() {
    ...
    var platform = sysInfo.platformVersionName || sysInfo.platform; // 旧版无法重定义 window
...
function (module, exports, __webpack_require__) {
  ...
  exports.navigator = _navigator3["default"]; // export XMLHttpRequest from './XMLHttpRequest'
  // export WebSocket from './WebSocket'
  // export Image from './Image'

  exports.Audio = _Audio3["default"];
  exports.FileReader = _FileReader3["default"];
  exports.HTMLElement = _HTMLElement3["default"]; // export localStorage from './localStorage'

  exports.location = _location3["default"]; // 暴露全局的 canvas

  var canvas = new _Canvas2["default"]();
  exports.canvas = canvas; // export { setTimeout }
  // export { setInterval }
  // export { clearTimeout }
...
function (module, exports, __webpack_require__) {
  ...
  var _util = __webpack_require__(9); // TODO 需要 qg.getSystemInfo 获取更详细信息
...
if (window.__globalAdapter) {
  ...
  utils.cloneMethod(globalAdapter, qg, 'getSystemInfoSync'); // TouchEvent
  ...
  utils.cloneMethod(globalAdapter, qg, 'createInnerAudioContext'); // FrameRate

  utils.cloneMethod(globalAdapter, qg, 'setPreferredFramesPerSecond'); // Keyboard
  ...
  utils.cloneMethod(globalAdapter, qg, 'offKeyboardComplete'); // Message

  utils.cloneMethod(globalAdapter, qg, 'getOpenDataContext');
  utils.cloneMethod(globalAdapter, qg, 'onMessage'); // Subpackage

  utils.cloneMethod(globalAdapter, qg, 'loadSubpackage'); // SharedCanvas

  utils.cloneMethod(globalAdapter, qg, 'getSharedCanvas'); // Font

  utils.cloneMethod(globalAdapter, qg, 'loadFont'); // hide show Event

  utils.cloneMethod(globalAdapter, qg, 'onShow');
  utils.cloneMethod(globalAdapter, qg, 'onHide'); // Accelerometer
  // NOTE: There is no LANDSCAPE_LEFT on Xiaomi platform
  // const LANDSCAPE_LEFT = -90;

  var LANDSCAPE_RIGHT = 90; // NOTE: the data in callback registered on onAccelerometerChange is 10 times than the standard data
