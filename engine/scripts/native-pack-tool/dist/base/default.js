...
class NativePackTool {
    ...
    appendCmakeResDirArgs(args) {
        args.push(`-DRES_DIR="${utils_1.cchelper.fixPath(this.paths.buildDir)}" -DAPP_NAME="${this.params.projectName}" `);
    }
    /**
    ...
    async encrypteScripts() {
        ...
        for (const configPath of allBundleConfigs) {
            ...
            if (this.params.compressZip) {
                content = zlib_1.gzipSync(content);
