...
class IOSPackTool extends mac_os_1.MacOSPackTool {
    ...
    selectSimulatorId() {
        const iphones = child_process_1.execSync('xcrun xctrace list devices')
            .toString('utf-8')
            .split('\n')
    ...
    selectIosDevices() {
        const lines = child_process_1.execSync(`xcrun simctl list`)
            .toString('utf-8')
            .split('\n');
    ...
    queryIosDevice() {
        const lines = child_process_1.execSync(`xcrun xctrace list devices`)
            .toString('utf-8')
            .split('\n');
        const ret = [];
        // skip first line
    ...
    async runIosDevice() {
        ...
        const foundApps = child_process_1.execSync(`find "${buildDir}" -name "*.app"`)
            .toString('utf-8')
            .split('\n')
