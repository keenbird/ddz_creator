...
export class UIScrollView extends Component {
    ...
    spacing = 0; //item之间的间隔
    @property
    bufferZone = 0; //每个item位置超过该数值变化,不填写则为scrollView高度/宽度的一半+itemTemplate高度/宽度(不可少于一半+item高宽，totalCount越大可设置越大)
    @property
    totalCount = 0; //可展示item的总数,不填写则自动计算scrollView可容纳的item数目 + 4
    @property
    topSpacing = 0; //初始化时第一个item离顶点的距离,需小于item高度的1/2
    @property({ type: ScrollView, displayName: "scrollView" })
    scrollView: ScrollView;
    @property({ type: ccNode, displayName: "item" })
    itemTemplate: ccNode;
    ...
    public init(param: SVNeedData): void {
        ...
        if (itemHeightWidth == 0) {
            fw.printWarn("itemHeightWidth can't set zero");
            return;
        }
