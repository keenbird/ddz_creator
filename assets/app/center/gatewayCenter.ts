...
export class GatewayCenter extends GatewayMainInetMsg {
    /**命令ID */
    cmd = proto.gateway_command.GS_GATEWAY_COMMAND_MSG
    /**网络延迟 发收一次的时间*/
    ...
    initRegister() {
        ...
        this.bindMessage({
            printLog: false,
            struct: proto.gateway_command.keepactive_cs,
            cmd: this.cmd.GATEWAY_COMMAND_KEEPACTIVE,
    ...
    /**获取心跳包数据 */
    getHearbeat() {
        let data = proto.gateway_command.keepactive_cs.create()
        data.timer = app.func.time(true) * 1000
        return this.getSendData(this.cmd.GATEWAY_COMMAND_KEEPACTIVE, data);
    }
    /**提示 */
    ...
    /**接受的心跳包 */
    OnRecv_GatewayKeepActive(data: proto.gateway_command.Ikeepactive_cs) {
        this.mServerDelayTime = app.func.time(true) * 1000 - data.timer;
