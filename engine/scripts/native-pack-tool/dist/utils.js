...
class cchelper {
    ...
    static checkJavaHome() {
        ...
        if (!javaPath) {
            ...
            try {
                const version = child_process_1.execSync(`"${cchelper.fixPath(javaPath)}" -version`).toString();
...
exports.toolHelper = {
    getXcodeMajorVerion() {
        try {
            const output = child_process_1.execSync('xcrun xcodebuild -version').toString('utf8');
    ...
    runCmake(args) {
        ...
        return new Promise((resolve, reject) => {
            ...
            const cp = child_process_1.spawn(cmakePath, args, {
    ...
    runXcodeBuild(args) {
        ...
        return new Promise((resolve, reject) => {
            ...
            if (!xcodebuildPath) {
                ...
                console.log(`run xcodebuild with ${args.join(' ')}`);
                const cp = child_process_1.spawn(xcodebuildPath, args, {
                    stdio: ['pipe', 'pipe', 'pipe'],
                    env: newEnv,
                    shell: true,
                });
